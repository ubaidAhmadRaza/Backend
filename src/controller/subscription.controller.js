...
const toggleSubscription = asyncHandler(async (req, res) => {
    ...
    if (channelOwner?._id === req.user?._id) {
...
// controller to return subscriber list of a channel
    ...
    if(!channelOwner){
        ...
        const subscribers = await Subscription.find({ channel: channelOwner._id })
...
// controller to return channel list to which user has subscribed
    ...
    const getSubscribedChannels = asyncHandler(async (req, res) => {
        ...
        res.status(200).json(new ApiResponse(200, subscribedChannels, "Subscribed channels retrieved successfully", subscribedChannels));
    });
